...
$this->load->view('questionnaire/question-type/header', array('lbl' => $lbl));
    ...
    for ($l = 1; $l <= 2; $l++) {
        ...
        foreach(array("yn1", "yn2") as $var) {
            if(preg_match("/{$var}=([YN]{1})/", $answer, $matches)) {
                ${$var} = $matches[1];
            } else {
                ${$var} = '';
            }
        ...
        <form autocomplete="off" onsubmit="return false;">            
            <div class="range_slider_bg<?= $l == 2 ? ' hidenone' : "" ?>" id="question_box_<?= $l ?>" data-qid="<?= $question->id ?>">
                <input type="hidden" class="q_yn1" value="<?= $yn1 ?>"/>
                <input type="hidden" class="q_yn2" value="<?= $yn2 ?>"/>
                <div class="boolean_question centered shadow choose_yes_no yn1 col-sm-12 col-xs-12">
                    <a class="boolean_question_yes" data-id="Y"><?= lang("q_btn_yes"); ?></a>
                    <a class="boolean_question_no" data-id="N"><?= lang("q_btn_no"); ?></a>
                </div>
                <div id="otheryn<?= $l ?>" class="otheryn hidenone">
                    <div class="detail"><?=lang("{$lbl}s{$l}_subheading2");?></div>
                    <p class="hh-text"><?=lang("{$lbl}s{$l}_txt");?></p>
                    <div class="boolean_question centered shadow choose_yes_no yn2 col-sm-12 col-xs-12">
                        <a class="boolean_question_yes" data-id="Y"><?= lang("q_btn_yes"); ?></a>
                        <a class="boolean_question_no" data-id="N"><?= lang("q_btn_no"); ?></a>
                    </div>
                </div>
                    ...
                    <div class="hidenone spinner_row<?= $k ?>">
                        ...
                        <div class="rows_box">
                            <div class="spinner_box division_question_counting_5">
                                <ul class="spinners">
                                    <li>
                                        <div class="option" id="option<?= "{$l}_{$k}"?>"><?= lang("{$lbl}s{$l}_spin1"); ?></div>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $.fn.setDefault = function() {
            var yn = this.find(".q_yn1").val();
            this.find('.yn1 a[data-id="'+yn+'"]').removeClass('active').click();
        };
        
        $.fn.saveSelection = function(whose_answer, link) {
            var answer = new Array(), 
                yn1 = this.find(".q_yn1").val(),
                status = 0;
            if(yn1 == 'Y' || yn1 == 'N') {
                answer.push("yn1="+yn1);
            }
            if(yn1 == 'Y') {
                var yn = this.find(".q_yn2").val();
                if(yn == 'Y' || yn == 'N') {
                    answer.push("yn2="+yn);
                }
                if(yn == 'Y') {
        ...
        $(".yn1 a").click(function(e) {
            ...
            box.find('.q_yn1').val(choise);
            if(choise == 'Y') {
                box.find('.otheryn').removeClass('hidenone');
                var yn = box.find(".q_yn2").val();
                box.find('.yn2 a[data-id="'+yn+'"]').removeClass('active').click();
            } else {
                box.find('.otheryn, .op_rows, .spinner_row1, .spinner_row2').addClass('hidenone');
            }
        });
        
        $(".yn2 a").click(function(e) {
            e.preventDefault();
            toggleActiveClass(this);
            var choise = '';
            if($(this).hasClass('active')) {
