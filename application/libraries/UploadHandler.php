...
class UploadHandler
{
    ...
    protected function create_scaled_image($file_name, $version, $options) {
        if ($version == 'thumbnail') {
            return $this->create_thumbnails($file_name, $options);
        } else {
            if ($this->options['image_library'] === 2) {
    ...
    private function create_thumbnails($filename, $options) {
        ini_set("memory_limit","128M");
        list($file_path, $new_file_path) =
            $this->get_scaled_image_file_paths($filename, 'thumbnail');
        ...
        switch ($file['extension']) {
            case 'jpeg':
                $success = imagejpeg($thumb, $new_file_path, 90); //90 = jpeg quality
                break;
            case 'gif':
                $success = imagegif($thumb, $new_file_path);
                break;
            case 'png':
                $success = imagepng($thumb, $new_file_path);
                break;
            default:
                $success = imagejpeg($thumb, $new_file_path, 90); //90 = jpeg quality
                break;
