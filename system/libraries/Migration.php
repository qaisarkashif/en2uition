...
class CI_Migration {
	...
	public function version($target_version)
	{
		...
		for ($i = $start; $i != $stop; $i += $step)
		{
			...
			if (count($f) > 1)
			{
				$this->_error_string = sprintf(lang('migration_multiple_version'), $i);
				return FALSE;
			}
			...
			if (count($f) == 0)
			{
				...
				$this->_error_string = sprintf(lang('migration_not_found'), $i);
				return FALSE;
			}
			...
			if (preg_match('/^\d{3}_(\w+)$/', $name, $match))
			{
				...
				{
					$this->_error_string = sprintf(lang('migration_multiple_version'), $match[1]);
					return FALSE;
				}
			...
			{
				$this->_error_string = sprintf(lang('migration_invalid_filename'), $file);
				return FALSE;
			}
		}
